-- Phase 5 Slice 5.9: book_appointment_slot accepts optional expected mode and rejects mismatch.

DROP FUNCTION IF EXISTS public.book_appointment_slot(uuid);
...
REVOKE ALL ON FUNCTION public.book_appointment_slot(uuid, public.consultation_mode) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.book_appointment_slot(uuid, public.consultation_mode) FROM anon;
GRANT EXECUTE ON FUNCTION public.book_appointment_slot(uuid, public.consultation_mode) TO authenticated;

COMMENT ON FUNCTION public.book_appointment_slot(uuid, public.consultation_mode) IS
  'Atomically holds an open future slot as pending_payment (B2C prepaid). Copies slot.mode; optional p_mode must match when provided. payment_method defaults to online (clinic path in later slice).';
